...
void start_worker_mining(mining_worker_t *worker)
{
    ...
    CHECK(worker->kernel = clCreateKernel(worker->program, "blake3_hasher_mine", &err));

    size_t hasher_size = sizeof(blake3_hasher);
    // printf("==== context: %p %p %p %d %d %d\n", worker->context, worker->queue, worker->device_hasher, worker->platform_index, worker->device_index, worker->i);
    worker->device_hasher = clCreateBuffer(worker->context, CL_MEM_READ_WRITE, hasher_size, NULL, NULL);
    TRY(clEnqueueWriteBuffer(worker->queue, worker->device_hasher, CL_TRUE, 0, hasher_size, worker->hasher, 0, NULL, NULL));
    ...
    TRY(clEnqueueReadBuffer(worker->queue, worker->device_hasher, CL_TRUE, 0, hasher_size, worker->hasher, 0, NULL, &worker_completed));
